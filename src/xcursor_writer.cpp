...
void XcursorWriter::write_cursor(const std::vector<CursorImage>& images,
    ...
    // Clean up on exit
    auto cleanup = [&]() { XcursorImagesDestroy(xcur_images); };
    
    try {
        for (size_t i = 0; i < images.size(); ++i) {
            const auto& img = images[i];
            
            XcursorImage* xcur_img = XcursorImageCreate(
            ...
            // Convert RGBA to Xcursor format (ARGB, but actually BGRA in memory on LE)
            // Xcursor expects premultiplied ARGB in native byte order
            // On little-endian: bytes are B, G, R, A
            ...
            for (size_t p = 0; p < pixel_count; ++p) {
                ...
                xcur_img->pixels[p] = (static_cast<XcursorPixel>(a) << 24) |
                                      (static_cast<XcursorPixel>(r) << 16) |
                                      (static_cast<XcursorPixel>(g) << 8) |
                                      (static_cast<XcursorPixel>(b) << 0);
            }
...
void XcursorWriter::write_index_theme(const fs::path& theme_dir,
...
void XcursorWriter::create_aliases(const fs::path& cursors_dir,
                                    const std::string& primary_name,
                                    const std::vector<std::string>& aliases) {
    auto primary_path = cursors_dir / primary_name;
    
    if (!fs::exists(primary_path)) {
        spdlog::warn("Cannot create aliases: primary cursor '{}' does not exist", 
                     primary_name);
        return;
    }
