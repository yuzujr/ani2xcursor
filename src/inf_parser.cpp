...
std::string RegLineParser::parse_field(std::string_view line, size_t& pos) {
    ...
    while (pos < line.size() &&
           (line[pos] == ',' || std::isspace(static_cast<unsigned char>(line[pos])))) {
        if (line[pos] == ',') {
            ++pos;
            break;
        }
...
void InfParser::parse_impl(std::string_view content) {
    ...
    spdlog::info("INF parsed: theme='{}', {} cursor mappings, {} warnings", result_.theme_name,
                 result_.mappings.size(), result_.warnings.size());
...
std::string InfParser::expand_vars(std::string_view input) const {
    ...
    for (int iter = 0; iter < kMaxIterations; ++iter) {
        ...
        while (i < result.size()) {
            ...
            if (!var_name.empty() &&
                std::all_of(var_name.begin(), var_name.end(), [](unsigned char c) {
            ...
            if (auto it = variables_.find(var_name); it != variables_.end()) {
                ...
                if (iter == 0) {
                    const_cast<InfParser*>(this)->add_warning("Unresolved variable: %" + var_name +
                                                              "%");
                }
            }
...
void InfParser::add_mapping(const std::string& role, const std::string& value, bool high_priority) {
    ...
    // Otherwise, keep existing mapping (first Wreg wins, or first Scheme if no
    // Wreg)
}
...
std::optional<std::pair<std::string, std::string>> InfParser::parse_key_value(
