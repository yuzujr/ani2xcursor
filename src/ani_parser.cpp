...
Animation AniParser::parse_impl(std::span<const uint8_t> data) {
    ...
    spdlog::info("ANI: {} frames, {} steps, default rate {} jiffies ({}ms)",
                 anim.num_frames, anim.num_steps, anim.display_rate,
                 jiffies_to_ms(anim.display_rate));
...
void AniParser::parse_anih(const RiffChunk& chunk, Animation& anim) {
    ...
    spdlog::debug("ANI anih: cbSize={}, frames={}, steps={}, rate={}, flags={:#x}",
                  cb_size, anim.num_frames, anim.num_steps, anim.display_rate, anim.flags);
}
...
std::vector<AniFrame> AniParser::parse_frames(const RiffReader& reader,
                                               const RiffChunk& fram_list,
                                               uint32_t num_frames) {
    ...
    reader.iterate_chunks(fram_list.data, [&](const RiffChunk& chunk) {
        if (chunk.fourcc == "icon") {
            ...
            spdlog::debug("ANI: Found icon frame {} ({} bytes)", 
                         frames.size(), chunk.data.size());
        }
        return true;
    });
