...
std::vector<CursorImage> IcoCurDecoder::decode_all(std::span<const uint8_t> data) {
    ...
    for (size_t idx : indices) {
        ...
        spdlog::debug("ICO/CUR: Image #{}: {}x{}, offset={}, size={}", idx, w, h, entry.offset,
                      entry.size);
...
CursorImage IcoCurDecoder::decode_png(std::span<const uint8_t> data, uint16_t hotspot_x,
                                      uint16_t hotspot_y) {
    int w, h, channels;
    uint8_t* pixels =
    ...
    spdlog::debug("ICO/CUR: Decoded PNG {}x{}, hotspot ({}, {})", img.width, img.height,
                  img.hotspot_x, img.hotspot_y);

    return img;
}

CursorImage IcoCurDecoder::decode_bmp(std::span<const uint8_t> data, const DirEntry& entry,
                                      bool is_cursor) {
    ...
    spdlog::debug("ICO/CUR: Decoded BMP {}x{} {}bpp, hotspot ({}, {})", img.width, img.height, bpp,
                  img.hotspot_x, img.hotspot_y);
