...
private:
    ...
    static CursorImage decode_png(std::span<const uint8_t> data, uint16_t hotspot_x,
                                  uint16_t hotspot_y);

    // Decode BMP/DIB image
    static CursorImage decode_bmp(std::span<const uint8_t> data, const DirEntry& entry,
                                  bool is_cursor);
